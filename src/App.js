        ...
        <div style={{ width: '100%', maxWidth: 400 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 24, padding: 24, marginBottom: 16, boxShadow: '0 10px 40px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {EMPLOYEES.map(emp => (
                <button key={emp.id} onClick={() => { setCurrentUser(emp); setView('employee'); }} style={{ width: '100%', padding: 16, borderRadius: 16, border: '1px solid #e2e8f0', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', transition: 'all 0.2s' }}>
          ...
          <button onClick={() => { setCurrentUser({ name: 'Administrator', id: 'ADMIN' }); setView('admin'); }} style={{ width: '100%', padding: 16, borderRadius: 16, border: 'none', background: 'rgba(255,255,255,0.15)', backdropFilter: 'blur(10px)', color: '#fff', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
      ...
      <div style={{ minHeight: '100vh', background: '#f5f7fa' }}>
        ...
        <header style={{ background: '#fff', borderBottom: '1px solid #e2e8f0', padding: '16px 20px', position: 'sticky', top: 0, zIndex: 10 }}>
          <div style={{ maxWidth: 500, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button onClick={() => { setView('login'); setCurrentUser(null); setSelfieUrl(null); setIsClockedIn(false); }} style={{ padding: 10, borderRadius: 10, border: 'none', background: '#f1f5f9', cursor: 'pointer' }}>
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#f5f7fa' }}>
        ...
        <header style={{ background: '#fff', borderBottom: '1px solid #e2e8f0', padding: '16px 20px', position: 'sticky', top: 0, zIndex: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ width: 40, height: 40, borderRadius: 10, background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><MapPin style={{ width: 20, height: 20, color: '#fff' }} /></div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <button onClick={() => { setView('login'); setCurrentUser(null); }} style={{ padding: 10, borderRadius: 10, border: 'none', background: '#f1f5f9', cursor: 'pointer' }}><LogOut style={{ width: 20, height: 20, color: '#64748b' }} /></button>
            </div>
          </div>
        </header>
            ...
            {attendanceLog.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>{attendanceLog.slice(0, 10).map(log => (
                <div key={log.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 12, borderRadius: 12, background: '#f8fafc' }}>
                  ...
                  <div style={{ flex: 1 }}><p style={{ fontWeight: 500, color: '#1e293b', margin: 0, fontSize: 14 }}>{log.employeeName}</p><p style={{ color: '#64748b', fontSize: 12, margin: 0 }}>{log.type.replace('-', ' ')}</p></div>
